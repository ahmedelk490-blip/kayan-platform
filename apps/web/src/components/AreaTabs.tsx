...
export function AreaTabs({ groups }: { groups: AreaGroup[] }) {
  ...
  return (
    <div className="border-b border-line bg-card px-4 lg:px-8">
      <nav
        ...
        {active.links.map((l) => {
          ...
          return (
            <Link
              ...
              className={
                isActive
                  ? 'shrink-0 border-b-2 border-brand px-3 py-2.5 text-[0.8125rem] font-medium text-brand lg:px-4 lg:py-3 lg:text-sm'
                  : 'shrink-0 border-b-2 border-transparent px-3 py-2.5 text-[0.8125rem] text-txt-3 transition-colors hover:text-brand lg:px-4 lg:py-3 lg:text-sm'
              }
            >
              {l.label}
            </Link>
