      ...
      <div className="space-y-6">
        ...
        <Panel title="المستخدمون">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-ink-800 text-start text-xs text-neutral-500">
              ...
              <tbody className="divide-y divide-ink-800">
                {users.map((entry) => (
                  <tr key={entry.id}>
                    <td className="py-3 text-neutral-200">{entry.nameAr ?? entry.name}</td>
                    <td dir="ltr" className="py-3 text-start text-neutral-400">
                      {entry.email}
                    </td>
                    <td className="py-3 text-neutral-400">{entry.role.nameAr}</td>
                    <td className="tnum py-3 text-neutral-500">
                      {entry.lastLoginAt
                    ...
                    <td className="py-3">
                      <span
                        className={
                          entry.isActive
                            ? 'rounded-full bg-success-600/15 px-2.5 py-1 text-[0.7rem] text-success-500'
                            : 'rounded-full bg-danger-600/15 px-2.5 py-1 text-[0.7rem] text-danger-500'
                        }
                      >
        ...
        <Panel title="الأدوار والصلاحيات">
          <ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {roles.map((role) => (
              <li key={role.id} className="rounded-lg border border-ink-800 p-4">
                <p className="text-sm text-neutral-200">{role.nameAr}</p>
                <p className="tnum mt-1.5 text-xs text-neutral-500">
                  {role._count.permissions} صلاحية · {role._count.users} مستخدم
                </p>
                <p dir="ltr" className="mt-2 text-start text-[0.7rem] text-neutral-600">
                  {role.landingPath}
                </p>
              </li>
            ))}
