...
/**
 * إدارة المنتجات — قائمة، بحث، فلترة بالتصنيف، وإنشاء/تعديل (مع نظام الدست).
 *
 * تُركَّب في مسارين: الكتالوج (`/catalog/products`) وتبويب المخزن
 * (`/inventory/products`)، فالمنتجات مكانها واحد بصرفٍ عن الباب الذي دخلت منه.
 * `basePath` يوجّه روابط الصفحة (الترقيم، شرائح التصنيف) للمسار الحالي حتى لا
 * يقفز المستخدم بين المجموعتين. تعديل التفاصيل (الألوان/الصور) يبقى في الكتالوج.
 */
export async function ProductsArea({
  searchParams,
  basePath,
  title = 'المنتجات',
}: {
  ...
  return (
    <AppShell user={user} title={title}>
      ...
      <Pager basePath={basePath} query={query} count={count} />
    </AppShell>
  );
}
