/**
 * ⚠️ أثري — لا تستعمله للنشر (2026-09-01).
 *
 * يستهدف بنية `~/domains/…/app/` التي حُذفت من الخادم في 18 أغسطس. النشر
 * الفعّال الآن: `git push` فيبني هوستنجر تلقائياً — راجع DEPLOY_GITHUB.md.
 * أُبقي لأن مراحله المحلية (lint → typecheck → تحزيم → بناء الحزمة المسطّحة)
 * ما زالت أدقّ محاكاة لبيئة هوستنجر قبل الدفع.
 *
 *   node scripts/deploy-site.mjs          نشر (المرحلة البعيدة ستفشل — المسار محذوف)
